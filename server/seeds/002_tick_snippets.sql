-- Tick content. Every snippet has exactly ONE defensible output order.
--
...
CREATE OR REPLACE FUNCTION seed_tick(
    p_prompt TEXT,
...
DECLARE
    ...
    v_pos INTEGER := 0;
BEGIN
    IF EXISTS (SELECT 1 FROM tick_snippets WHERE prompt = p_prompt) THEN
        RETURN;
    END IF;

    INSERT INTO tick_snippets (prompt, difficulty)
    VALUES (p_prompt, p_difficulty)
    RETURNING id INTO v_id;

    FOREACH v_text IN ARRAY p_outputs LOOP
        v_pos := v_pos + 1;
        INSERT INTO tick_outputs (snippet_id, output_text, position)
        VALUES (v_id, v_text, v_pos);
    END LOOP;

    FOREACH v_text IN ARRAY p_distractors LOOP
        INSERT INTO tick_outputs (snippet_id, output_text, position, is_distractor)
...
DROP FUNCTION seed_tick(TEXT, SMALLINT, TEXT[], TEXT[]);
...
FROM tick_snippets s
JOIN tick_outputs o ON o.snippet_id = s.id
